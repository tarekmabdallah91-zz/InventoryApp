...
class ProductDbHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        final String CREATE_TABLE_PRODUCT =
                CREATE_TABLE + ProductEntry.TABLE_NAME + OPEN_BRACKET +
                        ...
                        ProductEntry.COLUMN_PRODUCT_IMAGE3 + BLOB + COMA +
                        ProductEntry.COLUMN_PRODUCT_SYNCED + INTEGER + NOT_NULL + DEFAULT + ZERO + COMA +
                        ProductEntry.COLUMN_PRODUCT_FAVORED + INTEGER + NOT_NULL + DEFAULT + ZERO + CLOSE_BRACKET + SEMI_COLUMN;
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        db.execSQL(DROP_TABLE_IF_EXISTS + ProductEntry.TABLE_NAME);
        onCreate(db);
    }
}
